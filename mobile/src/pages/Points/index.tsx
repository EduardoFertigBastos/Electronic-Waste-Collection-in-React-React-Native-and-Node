...
const Points = () => 
{
  ...
  const sUrl = 'http://192.168.0.112:3333/';
  ...
  useEffect(() => {
    async function loadPosition() 
    {
      ...
      if (status !== 'granted')
      {
        Alert.alert('Calma ai, meu colega', 'Libera a localização, meu bruxo');
        return;
      }
  ...
  useEffect(() => 
  {
    api.get(`${sUrl}items`).then(response => {
    ...
    <>
      <View style={styles.container}>
        ...
        <View style={styles.mapContainer}>          
          { 
            // Estrutura similar a IF(initialPosition[0] !== 0) {Depois do &&}
            initialPosition[0] !== 0 && (
